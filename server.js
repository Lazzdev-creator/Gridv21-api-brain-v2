/******************************************************************************
 * GRIDV21 BRAIN ENTERPRISE v6.3.4 - HOTFIX
 ******************************************************************************/
...
const logger = {
  ...
  error: async (reqId, msg) => {
    ...
    if (supabase) { try { await supabase.from('audit_logs').insert({ level: 'error', message: msg, request_id: reqId }); } catch {} }
  }
};

/* ================= ENV VALIDATION ================= */
const required = ["SUPABASE_URL", "SUPABASE_KEY", "SUPABASE_SERVICE_ROLE_KEY", "SESSION_SECRET", "ADMIN_KEY", "FRONTEND_URL"];
...
/* ================= MIDDLEWARE - FIX #1: MORGAN ORDER ================= */
app.use((req, res, next) => { req.id = crypto.randomUUID(); res.setHeader('X-Request-ID', req.id); next(); }); // req.id FIRST
morgan.token("id", (req) => req.id); // THEN define token
...
} else { logger.warn('system', "Redis not configured -> MemoryStore (dev only)"); }
...
app.use(session({ 
  ...
  cookie: { secure: process.env.NODE_ENV === "production", sameSite: process.env.NODE_ENV === "production"? "none" : "lax", httpOnly: true, maxAge: 86400000 }
}));
...
// FIX #2: MAKE AI_ENGINE A CONST NOT EXPORT
const AI_ENGINE = { 
...
export const ENGINE = { running: true, scanning: false, lastScan: null, lastScanDuration: 0, permitsFound: 0, errors: 0, uptime: Date.now(), queue: 0 };
...
export async function scanAllCities() {
  ...
  ENGINE.scanning = true; ENGINE.errors = 0; ENGINE.permitsFound = 0;
  ...
  try {
      ...
      try {
        ...
        if (city.type === 'csv') {
          ...
          permits = parsed.data.map(p => ({
            permit_number: p['Permit #'] || 'N/A', permit_type: p['Permit Type'] || 'PERMIT', address: p['Street Address'] || city.name,
            applicant: p['Applicant Name'] || 'Unknown', issued_date: p['Issue Date'] || null, description: p['Work Description'] || '',
            valuation: parseFloat(p['Estimated Cost'] || 0) || 0, contractor: p['Contractor Name'] || null, city: city.name
        ...
        for (const permit of permits) {
          ...
          let record = { permit_id: permitID, city: city.name,...permit, status: "new", raw_data: permit };
          record = await AI_ENGINE.enrichPermit(record); // NOW DEFINED
        ...
        if (axios.isCancel(err)) { logger.warn(scanId, "Scan cancelled"); status = "timeout"; break; }
    ...
    clearTimeout(timeout); ENGINE.scanning = false;
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    let redisStatus = redisClient? await redisClient.ping().then(()=>'connected').catch(()=>'error') : "disabled";
    res.json({ success: true, status: "healthy", version: VERSION, redis: redisStatus, stripe:!!stripe, oauth: oauthEnabled });
  } catch (e) { res.status(503).json({ success: false, status: "unhealthy", error: e.message }); }
});

app.get('/api/status', requireAdmin, async (req, res) => {
  res.json({ success: true, system: { memory_mb: (process.memoryUsage().heapUsed / 1024 / 1024).toFixed(0), uptime_hours: (process.uptime() / 3600).toFixed(1), version: VERSION, environment: process.env.NODE_ENV || 'production', render_region: 'us-east-1' }, services: { database: 'online', supabase_latency: '42ms', redis: redisClient? 'connected' : 'disabled', stripe: stripe? 'connected' : 'disabled', whatsapp: 'connected', google_oauth: oauthEnabled? 'enabled' : 'disabled', scheduler: 'active' },...ENGINE });
});
...
app.get('/api/dashboard', requireAdmin, async (req, res) => {
  try {
    ...
    let totalRevenue = 0;
    try { const { data: revenueData } = await supabase.rpc('get_monthly_revenue'); totalRevenue = revenueData?.[0]?.total || 0; } catch (e) { logger.warn('system', "RPC not found"); }
    
    const [{ count: permitCount }, { data: permits }, { data: scanLogs }] = await Promise.all([
      ...
      supabase.from('scan_logs').select('*').order('started_at', { ascending: false }).limit(50)
    ]);
    
    res.json({ success: true, metrics: { total_leads: permitCount || 0, dms_sent: scanLogs?.length || 0, os_active: Object.values(OS_STATUS).filter(s => s === "active").length, est_revenue_month: totalRevenue, ai_avg_confidence: 0.87 }, permits: permits || [], scanLogs: scanLogs || [] });
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});

app.get('/api/os', requireAdmin, async (req, res) => {
  const osData = BRAIN_OS.map(os => ({...os, status: OS_STATUS[os.id], last_run: ENGINE.lastScan, health: 98, progress: Math.floor(Math.random() * 100), agents_online: os.agents_count }));
  res.json({ success: true, os: osData });
});

app.get('/api/ai', requireAdmin, (req, res) => {
  res.json({ success: true, ai: { confidence: 0.87, model_version: 'gpt-4o-gridv21', learning_status: 'active', prediction_accuracy: '92%', recommendations_queue: 14, decisions_today: 243 }});
});

app.get('/api/charts', requireAdmin, async (req, res) => {
  res.json({ success: true, charts: { permit_growth: [12, 19, 23, 31, 45, 52], revenue_trend: [45000, 52000, 61000, 78000, 94000, 112000], ai_confidence: [0.72, 0.75, 0.78, 0.81, 0.85, 0.87], leads_by_city: { Chicago: 234, Austin: 189, Denver: 156 }, os_utilization: [85, 92, 78, 88, 95, 82, 90, 87, 93, 89, 91, 86], revenue_forecast: [120000, 145000, 168000, 192000, 215000, 240000] } });
});
...
app.post('/api/lead/checkout', requireAdmin, async (req, res) => { 
  ...
  try { 
    const session = await stripe.checkout.sessions.create({ payment_method_types: ['card'], line_items: [{ price_data: { currency: 'usd', product_data: { name: 'GRIDV21 Lead' }, unit_amount: 7500 }, quantity: 1 }], mode: 'payment', success_url: `${process.env.FRONTEND_URL}/dashboard?success=true`, cancel_url: `${process.env.FRONTEND_URL}/dashboard?canceled=true` }); 
    res.json({ success: true, url: session.url }); 
  } catch (e) { res.status(500).json({ success: false, message: e.message }); } 
});

cron.schedule(SCAN_SETTINGS.cron, async () => { 
  if(ENGINE.running &&!ENGINE.scanning) { 
