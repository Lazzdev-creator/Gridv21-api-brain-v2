console.log('GRIDV21 BRAIN v4.4.4d FINAL starting... Node:', process.version);
...
// === API ROUTES FIRST - THIS FIXES THE '<' ERROR ===
app.get('/api/metrics', async (req, res) => {
  const metrics = await Brain.getMetrics();
  res.json({
  ...metrics,
    user: { email: 'admin@gridv21.com', contacts: { YouTube: 'Not connected', LinkedIn: 'Not connected', WhatsApp: 'Not connected' }
  });
});
...
app.get('/api/forecast', async (req, res) => {
  const metrics = await Brain.getMetrics();
  res.json({ forecast_90days: metrics.est_revenue_month * 3, mode: metrics.mode })
});
...
// === STATIC FILES LAST - must be after all APIs ===
app.use(express.static(path.join(__dirname, 'public')));
app.get('/', (req, res) => res.sendFile(path.join(__dirname, 'public', 'dashboard.html')));
app.get('/dashboard.html', (req, res) => res.sendFile(path.join(__dirname, 'public', 'dashboard.html')));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`GRIDV21 BRAIN v4.4.4d FINAL on port ${PORT}`));
